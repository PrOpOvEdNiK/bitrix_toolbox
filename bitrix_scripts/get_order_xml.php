...
if (isset($_REQUEST['ORDER_ID']) || isset($_REQUEST['ORDER_ID_TEXT'])) {
    ...
    if ($nOrder && CSaleOrder::GetByID($nOrder)) {
        ...
        ob_start();
        CSaleExport::ExportOrders2Xml(['ID' => $nOrder]);
        $sXml = ob_get_clean();

        echo strtr($sXml, ['encoding="windows-1251"' => 'encoding="utf-8"']);
        exit();
    } else {
...
$arOrdersID = [];
$dbOrders = CSaleOrder::GetList(["ID" => "DESC"], [], false, ['nTopCount' => 200], ['ID', 'DATE_INSERT']);
